...
如 *函数执行上下文* 中所述，一般的函数调用会创建“函数执行上下文”并将其压入ECS栈顶，然而，对于 `eval()` 函数，这一过程有些特殊。
...
我们知道，作用域确定当前执行代码对变量的访问权限。在ECMAScript 3规范下，当需要查找变量时，会先从当前执行上下文的变量对象中寻找，若查找无果，则从父级执行上下文的变量对象中查找，若仍然无果，则继续向下查找，直到获取到该变量或已查找到全局执行上下文的变量对象（ `globalThis` ）。而作用域链就是以各级执行上下文的变量对象作为节点，并将其以一定顺序链接而组成的链表。
